...
function getResAjax()
{
	...
	$r = new Ressource();
	$lR = $r->getList(array('elt_type = ?' => 'pg', 'elt_id = ?' => $_GET['act_id']));
	if (!empty($lR))
	{
		foreach ($lR as $cR)
		{
...
function uploadRessource()
{
	...
	$r = new Ressource();
	$r->save('res_id', array('res_id' => null, 'elt_id' => $_GET['act_id'], 'elt_type' => 'pg', 'res_ordre' => 1, 'res_titre' => $originName, 'res_contenu' => $fileName, 'res_mime' => $extension));
	
	// Return Success JSON-RPC response
	die('{"jsonrpc" : "2.0", "result" : null, "id" : "id"}');
}

function duplicActualite()
{
	...
	else 
	{
		if(!empty($_POST['site_id'])){
			...
			{
				$p->duplique($_POST['act_id'], $site_id);
				$i++;
			}
			$_GET['act_id'] = $_POST['act_id'];
...
function listeActualite() 
{
	...
	$lElt = $elt->getList($clause, 'act_date DESC');	
	
	$tpl->assign(array('lElt' => $lElt, 'nb' => sizeof($lElt)));	
	
	if (!empty($_GET['act_id']))
	{
		$cActualite = new Actualite($_GET['act_id']);	
		$tpl->assign(array("cActualite" => $cActualite));
	...
	getListeCategorie();	
	getListeStatus();	
	$lTpl[] = "contenu/tpl/adm_actu_liste.tpl";
}
...
function saveActualite()
{
	...
	//print_r($_POST);
	$_POST['act_photo'] = upload('act_photo', 'act_photo_old', $CFG->imgpath, array("p_" => array(550), 'vign_' => array(50)));
	if (empty($_POST['act_date'])) $_POST['act_date'] = time();
	else $_POST['act_date'] = date_fr_to_timestamp($_POST['act_date']);	
