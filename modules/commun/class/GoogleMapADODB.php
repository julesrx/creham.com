 ...
 * @version 3.0beta
 */
class GoogleMapAPI  extends ObjectModel 
{
	protected $table = 'geocodes';
	
	protected $fields = array('address','lon', 'lat');		
    ...
    function createMarkerIcon($iconImage,$iconShadowImage = '',$iconAnchorX = 'x',$iconAnchorY = 'x',$infoWindowAnchorX = 'x',$infoWindowAnchorY = 'x') {
        ...
        $_image_info = array(31, 31); // TRIPTIC
	...
	*/
    ...
    function putCache($address, $lon, $lat) {
        if(!isset($this->dsn) || (strlen($address) == 0 || strlen($lon) == 0 || strlen($lat) == 0))
           return false;
        $sql = 'insert into '.$this->_db_cache_table.' values (?, ?, ?)';
        $p = Db::get()->Prepare($sql);
	    $rs = Db::get()->Execute($p,array($address, $lon, $lat));	 
